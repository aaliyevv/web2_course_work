package com.example.subwaycustomer.aspect;
...
public class LoggingAspect {
    @Before("execution(* com.example.subwaycustomer.controller.*.*(..))")
    ...
    @AfterReturning(pointcut = "execution(* com.example.subwaycustomer.controller.*.*(..))", returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        System.out.println("Method " + joinPoint.getSignature().getName() + " executed with return value: " + result);
    }

    @Around("execution(* com.example.subwaycustomer.controller.*.*(..))")
